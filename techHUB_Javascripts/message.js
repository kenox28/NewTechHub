...
btnPost.onclick = function (e) {
	...
	xhr.open("POST", "../newPhpfileTechhub/chatmessage.php", true);
	xhr.onload = function () {
...
convodiv.onmouseenter = function (e) {
	e.preventDefault();
	convodiv.classList.add("active2");
};

convodiv.onmouseleave = function (e) {
	e.preventDefault();

	convodiv.classList.remove("active2");
};

setInterval(function () {
	let xhr = new XMLHttpRequest();

	xhr.open("POST", "../newPhpfileTechhub/convo.php", true);
	xhr.onload = function () {
		if (xhr.readyState === XMLHttpRequest.DONE) {
			if (xhr.status === 200) {
				...
				if (!convodiv.classList.contains("active2")) {
					tosrolauto();
				}
			}
		}
